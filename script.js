function convertCard(card) {
    ...
    const suits = {
      's': { symbol: '♠', color: '#888888' },  // soft gray for dark background
      'h': { symbol: '♥', color: '#FF0000' },  // red
      'd': { symbol: '♦', color: '#0000FF' },  // blue
      'c': { symbol: '♣', color: '#008000' }   // green
    };
  
    const { symbol, color } = suits[suit] || { symbol: '?', color: '#000000' };
  
    return `[color=${color}]${value}${symbol}[/color]`;
  }
...
function convertHand() {
    ...
    if (blindLine) {
      ...
      if (match) {
        // const sbAmount = parseFloat(match[1]); // optional if needed
    ...
    for (const line of lines) {
      ...
      // add copy formatted hand history to clipboard
  ...
  const actionMap = preflopLines.map(line => {
    ...
    if (actionPart.includes('raises')) {
      ...
      if (toMatch) {
        ...
        pot += amount;
        return `[color=#C51F1F]${player} raises to $${(amount / bbAmount).toFixed(2)}[/color]`;
      }
    }
    if (actionPart.includes('calls')) {
      const amount = parseInt(actionPart.match(/calls (\d+)/)?.[1] || 0);
      pot += amount;
      return `${player} calls $${(amount / bbAmount).toFixed(2)}`;
    }
  ...
  preflopText += `($${(pot / bbAmount).toFixed(2)})    \n` + actionMap.join(', ');
...
function parseStreet(streetName, previousPot) {
    ...
    for (const line of streetLines) {
      if (line.includes("bets")) {
        ...
        if (match) {
          ...
          pot += amount;
          actions.push(`[color=#C51F1F]${player} bets $${(amount / bbAmount).toFixed(2)}[/color]`);
        }
        ...
        if (match) {
          ...
          actions.push(`[color=#C51F1F]${player} raises to ${isAllIn ? "all-in " : ""}$${(amount / bbAmount).toFixed(2)}[/color]`);
        }
        ...
        if (match) {
          ...
          const isAllIn = line.includes("is all-in") || line.includes("and is all-in");
          actions.push(`${player} calls ${isAllIn ? "all-in " : ""}$${(amount / bbAmount).toFixed(2)}`);
        }
    ...
    const text = `\n\n[b]${streetName}:[/b] ($${(pot / bbAmount).toFixed(2)}) ${streetCards} [color=#009B00](2 players)[/color]\n` + actions.join(', ');
    return { text, pot };
  }
